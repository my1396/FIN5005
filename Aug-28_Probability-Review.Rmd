...
$$
This is known as the change of variable formula for density functions.`||` denotes the absolute value of the differential. 
...
- The sample analogue of the expected value is the <span style='color:#008B45FF'>sample mean</span> or <span style='color:#008B45FF'>sample average</span>.
    ...
    $$
- Sample mean has some desirable properties as an style='color:#008B45FF'>estimator</span> of the expected value of a random variable.
...
The variance is obtained by setting $g(X) = \mathbb{E}\left[\left(X − \mathbb{E}(X)\right)^2\right]$:
$$
\begin{aligned}
...
The sample analogue of the variance for a sample of observations $X_1,...,X_n$ is the sample variance. One usually uses either
$$
...
$$
\mu_r = \mathbb{E}\left\{\left[\frac{X-\mathbb{E}}{\text{sd}(X)}\right]^r\right\}
$$
...
- Suppose that $Y_1, Y_2, ...,$ Yn are independent and identically distributed random variables; note that they have the same expected value $E(Y_i)$ and the same variance $\text{Var}(Y_i)$ for $i = 1, 2, ..., n.$
