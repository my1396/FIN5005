...
\begin{aligned}
\tilde{\beta_1} &= \beta_1 \\
\tilde{\beta_2} &= \beta_2+a \\
\tilde{\beta_3} &= \beta_3+a \\
\tilde{\beta_4} &= \beta_4-a \\
\end{aligned}
$$
...
Observed value $y$ corresponds to error term $\varepsilon$:
$$
y=X\beta + \varepsilon
$$
...
1. $x_1'e=0$, where $x_1$ is a $n\times 1$ column vector of 1, indicating the sum of the residuals is zero, i.e., $\sum_{i=1}^n e_i =0$.
...
3. By construction, the OLS residuals $\boldsymbol{e}$ in our sample are <span style='color:#337ab7'>uncorrelated</span> with each of the $K$ explanatory variables included in the model.
    ...
    &= \boldsymbol{0} - E[x_k] \boldsymbol{0}  \qquad (E[e]=0 \text{ as } x_1'e=0) \\
    &= 0
    \end{align*} 
    $$


4. $\overline{y}= \overline{x}\hat{\beta}$. This follows from the property $\overline{e}=0$.
$$
e=y-X\hat{\beta}
$$
...
5. The predicted values of $\hat{y}$ are uncorrelated with the residuals.
$$
\begin{aligned}
\hat{y}'e &= (X\hat{\beta})'e \\
&= \hat{\beta}'X'e \\
&= 0 \qquad (X'e=0)
\end{aligned}
$$
...
3. **Exogeneity of the independent variables**
    ...
    > This is not likely to be true in a time-series setting. The observation at $t$ might provide information about $E[\varepsilon_{t+1}]$, that is $E[\varepsilon_{t+1} \vert x_t} \ne 0$. This is called autocorrelation. 


    The zero conditional mean implies that the unconditional mean is also zero. By the Law of Iterated Expectation (LIE),
    
    $$
    E[\boldsymbol{\varepsilon}] = E_\boldsymbol{X}[E[\boldsymbol{\varepsilon} \vert \boldsymbol{X}]] = E_\boldsymbol{X}[\boldsymbol{0}] = \boldsymbol{0}
    $$
...
4. **Homoskedasticity and no autocorrelation**
    ...
    \begin{bmatrix} 
    E[\epsilon_1^2 \vert \boldsymbol{X} ] & E[\epsilon_1\epsilon_2 \vert \boldsymbol{X}] & \cdots  & E[\epsilon_1\epsilon_n \vert \boldsymbol{X}] \\
    E[\epsilon_2\epsilon_1 \vert \boldsymbol{X} ] & E[\epsilon_2^2 \vert \boldsymbol{X}] & \cdots  & E[\epsilon_2\epsilon_n \vert \boldsymbol{X}] \\
    \vdots & \vdots & \ddots & \vdots \\
    E[\epsilon_n\epsilon_1 \vert \boldsymbol{X} ] & E[\epsilon_n\epsilon_2 \vert \boldsymbol{X}] & \cdots  & E[\epsilon_n^2 \vert \boldsymbol{X}] \\
    \end{bmatrix}  \\
    &= \begin{bmatrix}
    \sigma^2 & 0 & \cdots & 0 \\
    0 & \sigma^2 & \cdots & 0 \\
    ...
    $$
    \text{Var}[\epsilon\vert \boldsymbol{X}] = \sigma^2
    $$
    for all $i=1,\ldots,n.$
    Constant variance is labeled <span style='color:#008B45FF'>**homoskedasticity**</span>.
    
    - Covariance of the disturbances:
    $$
    \text{Cov}[\epsilon_i, \epsilon_j\vert \boldsymbol{X}] = 0
    $$
    ...
    In case of homoskedasticity and nonautocorrelation, we have $\Omega=\sigma^2\boldsymbol{I}.$ And we call the disturbances spherical disturbances.
    
5. **Normal distribution of disturbances**

    $\boldsymbol{e}\vert \boldsymbol{X} \sim N(\boldsymbol{0}, \sigma^2\boldsymbol{I})$. We assume that the disturbances are normally distributed, with zero mean and constant variance.
...
We call models satisfying assumptions 1--5 as the "Classical Linear Regression Model."
...
$$
\begin{aligned}
\text{Var}(\boldsymbol{\hat{\beta}} \vert X) &= E[(\hat{\boldsymbol{\beta}}-\boldsymbol{\beta})(\hat{\boldsymbol{\beta}}-\boldsymbol{\beta})^T \vert \boldsymbol{X}] \\
&= E[(X'X)^{-1}X'\varepsilon\varepsilon'X(X'X)^{-1} \vert \boldsymbol{X}] \\
&= (X'X)^{-1}X'\, E[\boldsymbol{\varepsilon\varepsilon}'\vert \boldsymbol{X}] \, X(X'X)^{-1} \\
...
$$
\boldsymbol{V}_{\hat{\boldsymbol{\hat{\beta}}}} = \text{Var}(\boldsymbol{\hat{\beta}} \vert X) = \sigma^2(X'X)^{-1}
$$
