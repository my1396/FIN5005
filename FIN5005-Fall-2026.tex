...
\usepackage{xcolor}
% define colors
\newcommand{\red}[1]{\textcolor[RGB]{185,64,71}{#1}}
\newcommand{\green}[1]{\textcolor[RGB]{0, 139, 69}{#1}}
...
% unicode-math is already loaded by quarto's template (the \ifPDFTeX \else
% branch), which runs before this file is inserted. Loading it again here would
% work but risks an option clash if either side ever passes options.
\setmathfont{XCharter-Math.otf} % do NOT delete the .otf extension

% Make \boldsymbol work again. amsmath's \boldsymbol switches to a bold *math
% version*, which an OpenType math font does not provide — it fails silently,
% leaving the symbol unbolded and only a
%   LaTeX Font Warning: Font shape `TU/XCharter-Math.otf(1)/b/n' undefined
% in the log. unicode-math's \symbfit is the equivalent that works.
%
% Doing it here, once, keeps every \newcommand{\bX}{\boldsymbol{X}} below
% unchanged, and keeps those definitions identical to the \boldsymbol ones in
% themes/mathjax.html, so HTML and PDF stay in step.
%
% \symbfit, not \symbf: \symbfit is bold *italic*, which is what \boldsymbol
% has always meant in LaTeX and what MathJax renders in the HTML build.
% \symbf would give upright bold Latin (𝐗) and so differ from the web version.
%
% \AtBeginDocument so this does not depend on package load order: it runs once
% every package has been loaded, so it wins regardless of which package defined
% \boldsymbol last, and regardless of where in this file the line sits.
%
% \DeclareRobustCommand, not \renewcommand: the original \boldsymbol is robust
% (its meaning is "\protect\boldsymbol ", the \DeclareRobustCommand form), and
% \renewcommand would quietly make it fragile. A heading like
%   ## Estimating $\bX$ by OLS
% would then write the *expansion* (\symbfit) into the .toc/.aux rather than
% \boldsymbol itself, which breaks when those files are read back by a build
% where \symbfit is not defined.
\AtBeginDocument{\DeclareRobustCommand{\boldsymbol}[1]{\symbfit{#1}}}
...
%% support for Chinese (lualatex, not xelatex, so use luatexja-fontspec instead of xeCJK)
\usepackage{luatexja-fontspec}
\setmainjfont{LXGW WenKai}
\setsansjfont{Microsoft YaHei}
\setmonojfont{Maple Mono NF CN}
...
\renewcommand\mc[1]{\multicolumn{1}{c}{#1}} % center table header (overrides luatexja's CJK mincho-switch shortcut, unused here)
\usepackage{dcolumn}
...
and competencies to make data driven decisions.
...
\item
  ...
  constraints. E.g., determine the optimal price for a product to
  maximize profit.
\item
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.6146}}@{}}
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Descriptive} & Descriptive Analysis, Data Visualization & Asset
return → \textbf{Finance} California school expenditure and academic
performance → \textbf{Education} \\
\textbf{Predictive} & Linear regression & Asset return prediction
(Factor models) → \textbf{Finance}Test Score Determinants →
\textbf{Sociology} \\
& Hypothesis testing & Test Score determinants → \textbf{Sociology} \\
& Classification, logistic regression & Boston housing value
determinants → \textbf{Finance} \\
\end{longtable}
}
...
\item
  ...
  You can download the lecture notes in PDF format for offline reading.
  Click on the icon in the top-left corner to download the lecture notes
  in PDF format.
\item
...
website and StudentWeb, {\textbf{ALWAYS use the dates on StudentWeb as
the reference.}}

\begin{itemize}
\item
  \textbf{Hjemmeoppgave} (mandatory home assignment): 40\%
  ...
  Period: Weeks 40--42 (subject to change)\\
  Duration: 2 weeks.

  The assignment will be released in WISEflow, and you will have
  \textbf{two weeks} to complete it.

  More details will be provided as the course progresses.

  Everything you need to know about the home assignment in WISEflow:
  \href{https://www.nord.no/en/student/examinations-and-assessment/take-home-examinations-assignments-and-portfolios}{Take-home
  examinations, assignments/theses and portfolios}
\item
  \textbf{Skoleeksamen} (school exam): 60\%

  Digitally in WISEflow;\\
  Date: Refer to
  \href{https://www.nord.no/student/studentverktoy/studentweb}{StudentWeb}

  Everything you need to know about the shool exam in WISEflow:
  \href{https://www.nord.no/en/student/examinations-and-assessment/written-on-site-examinations}{Written
  on-site examinations}

  \begin{itemize}
  \tightlist
  \item
    You must provide your own PC or Mac with administrator rights.
  \item
    You need to download the Lockdown Browser.
  \item
    Perform a demo test to get familiar with WISEflow.
  \end{itemize}
\end{itemize}
...
\begin{tabular}{@{}ll@{}}
Instructor:    & Menghan Yuan \\[2pt]
Email:         & \href{mailto:menghan.yuan@nord.no}{menghan.yuan@nord.no} \\[2pt]
...
with both cloud-based and local options available.

\begin{itemize}
\tightlist
\item
  \textbf{Cloud option:}
  \href{https://colab.research.google.com/}{Google Colab with R Kernel}
  (Course Labs will be run in this)
\item
  \textbf{Local option:} \hyperref[RStudio-Desktop]{RStudio Desktop}
  (You will most likely use this for the home assignment)
\end{itemize}

At the end of the session, you will have two exercises to learn R basics
using Google Colab
\includegraphics[height=1.5em]{images/icon-google-colab.png} and RStudio
desktop.
...
However, convenience comes at the cost of performance and control. Below
is a comparison between cloud and local solutions.
...
Performance & Slower; up to the server & Faster; dependNon your
computer \\
Environment control & Limited; when your session ends, your installed
packages will be removed & Full control; environment is persistent
across sessions; \\
Internet connection & Required & Not required \\
Storage & Limited; files need to be uploaded/downloaded & Files are
stored directly on your computer \\
Best for & Learning, experimenting, and small projects & Regular
coursework, larger datasets, and research projects \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{\texorpdfstring{Google Colab
\raisebox{-0.4em}{\includegraphics[height=1.5em]{images/icon-google-colab.png}}}{Google Colab }}\label{google-colab}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Create a new R-Notebook using the following link:}

  \href{https://colab.research.google.com/notebook\#create=true&language=r}{colab.research.google.com/notebook\#create=true\&language=r}

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/colab new notebook.png}}
\item
  \textbf{Type your first R code.}

  A notebook consists of cells, which can contain either code or text.
  Use the toolbar to add new cells.

  For example, type the following code in a code cell and click the play
  button to run it:
...
\end{Shaded}
  ...
  The code output will show the R version and the packages currently
  loaded in your session.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/colab sessionInfo.png}}

  Congratulations! You have successfully set up your cloud-based R
  environment using Google Colab. You can now start coding and exploring
  R without any installation hassles.
\end{enumerate}

\textbf{Further readings about Google Colab:}

\begin{itemize}
\item
  \href{https://www.kaggle.com/code/ybifoundation/introduction-to-google-colab-notebook\#Code-cells}{Introduction
  to Google Colab Notebook}

  This tutorial introduces how to upload/download data, sharing with
  others, editing cells, and more.
\end{itemize}
...
A: Click on the ``\textbf{Runtime}'' in the menu bar \textgreater{}
Select ``\textbf{Change runtime type}'' \textgreater{} Select
``\textbf{R}'' from the dropdown menu \textgreater{} Click
``\textbf{Save}''.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Install the R programming language}

  Download the R installer from the
  \href{https://cran.r-project.org}{CRAN website}. Choose the
  appropriate version for your operating system (Windows, macOS, or
  Linux) and follow the installation instructions.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/R installer.png}}
\item
  \textbf{Install RStudio IDE}

  Go to
  \href{https://docs.posit.co/ide/user/\#direct-downloads-open-source}{RStudio
  IDE Direct Download} and download the installer for your operating
  system (Windows, macOS, or Linux).
\item
  \textbf{Check the installation is working}

  After installing both pieces of software, open RStudio to check the
  installation. It should start, and R should run in the Console window,
  listing the R version.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/RStudio layout.png}}
\end{enumerate}

\subsection{RStudio User Interface}\label{rstudio-user-interface}

Read
\href{https://docs.posit.co/ide/user/ide/guide/ui/ui-panes.html}{THIS
guide} to learn about the RStudio user interface.

\begin{center}
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/rstudio-panes-labeled.png}
\end{center}

\subsection{Create and save a script}\label{create-and-save-a-script}

R scripts (\texttt{.R} or \texttt{.r} files) are text files that contain
R code.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Click on ``\textbf{File}'' → ``\textbf{New File}'' → ``\textbf{R
  Script}''.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_rscript.png}}
\item
  Once the file has opened, Click on ``\textbf{File}'' →
  ``\textbf{Save}''.
\item
  Specify a name.~The extension \texttt{.R} is automatically added.
\end{enumerate}

You can now write R code in the script and run it in the Console window.

\raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_rscript_cmd.png}}

Select the code you want to run and click on the ``\textbf{Run}'' button
in the toolbar, or use the keyboard shortcut \texttt{Ctrl\ +\ Enter}
(Windows) or \texttt{Cmd\ +\ Enter} (Mac).

\section{AI Coding Assistant}\label{ai-coding-assistant}

\textbf{What AI coding assistant can do for you:}

\begin{itemize}
\tightlist
\item
  \textbf{Code autocompletion}: generating suggestions while typing the
  code.
\item
...
\textbf{How to use AI coding assistant in Google Colab and RStudio:}

\begin{itemize}
\item
  Google Colab has built-in support for \textbf{Gemini}.
\item
  RStudio uses \textbf{GitHub Copilot} as the AI coding assistant.
  ...
  Copilot offers autocomplete-style suggestions as you code as ``ghost
  text''.

  Create a new R script in RStudio and type the following code in the
  script. As you type the comment
  \texttt{\#\ add\ the\ standard\ error}, Copilot will show a
  suggestion, shown in light grey ``ghost text''.

  \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/rstudio_copilot.png}
  ...
  \tightlist
  \item
    \href{https://docs.posit.co/ide/user/ide/guide/tools/copilot.html\#setup}{RStudio
    User Guide: GitHub Copilot Setup}: detailed instructions on how to
    set up GitHub Copilot in RStudio.
  \item
    \href{https://docs.posit.co/ide/user/ide/guide/tools/copilot.html\#using-copilot}{RStudio
    User Guide: GitHub Copilot How to Use}
  \end{itemize}
\end{itemize}
...
\subsection{Exercise 1: Work with R Notebook in Google
Colab}\label{exercise-1-work-with-r-notebook-in-google-colab}

Use
\href{https://colab.research.google.com/drive/1cpY6EGpNrqwIz-7ZITTQHZie3BPV-BpF?usp=sharing}{THIS
LINK} to open the R Notebook in Google Colab.

The Notebook looks like the following:

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/Colab lab0.png}

Note: You only have View access to the notebook. To edit it, you need to
make a copy of the notebook by clicking on ``\textbf{File}'' →
``\textbf{Save a copy in Drive}''. You can then edit the notebook in
your own Google Drive.

Follow the instructions in the notebook to complete the exercises. {When
you are done with the exercises}, you can download the notebook to your
local computer by clicking on ``\textbf{File}'' → ``\textbf{Download}''
→ ``\textbf{Download .ipynb}''.

You can also download the notebook as a PDF file by clicking on
``\textbf{File}'' → ``\textbf{Print}'' → ``\textbf{Save as PDF}''.
...
You need to edit the notebook in Google Colab and then download it for
submission. If you want to edit the notebook on your local computer, you
can use \href{https://jupyter.org/install}{JupyterLab or Jupyter
Notebook}. It requires extra installation and configuration. You may
follow online tutorials to set it up.

For the purpose of this course, we simply use Google Colab for its
simplicity to use out-of-the-box without any installation and
configuration.

\end{tcolorbox}

\subsection{Exercise 2: Work with R script in
RStudio}\label{exercise-2-work-with-r-script-in-rstudio}

Copy and paste the following code into a new R script in RStudio and
complete the exercises. Save the script as \texttt{lab0.R}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Instructions:}
\CommentTok{\# {-} Work directly in this .R script.}
\CommentTok{\# {-} For each task, write your R code below the instructions.}
\CommentTok{\# {-} Run the code and copy the output in the console into the comments section below each task.}

\CommentTok{\# ============================================================}
\CommentTok{\# }\AlertTok{TASK}\CommentTok{ 1: Create and use objects}
\CommentTok{\# ============================================================}
\CommentTok{\#}
\CommentTok{\# 1. Create an object called \textasciigrave{}price\textasciigrave{} and give it the value 100.}
\CommentTok{\# 2. Create an object called \textasciigrave{}quantity\textasciigrave{} and give it the value 5.}
\CommentTok{\# 3. Create an object called \textasciigrave{}total\textasciigrave{} that calculates price * quantity.}
\CommentTok{\# 4. Print \textasciigrave{}total\textasciigrave{}.}
\CommentTok{\#}
\CommentTok{\# Write your code below:}


\CommentTok{\# Copy and paste the relevant Console output below.}
\CommentTok{\# Output:}


\CommentTok{\# ============================================================}
\CommentTok{\# }\AlertTok{TASK}\CommentTok{ 2: Work with vectors}
\CommentTok{\# ============================================================}
\CommentTok{\#}
\CommentTok{\# A vector is a collection of values.}
\CommentTok{\#}
\CommentTok{\# 1. Create a vector called \textasciigrave{}prices\textasciigrave{} containing:}
\CommentTok{\#    10, 15, 20, 25, 30}
\CommentTok{\# 2. Calculate the mean of \textasciigrave{}prices\textasciigrave{}.}
\CommentTok{\# 3. Find the minimum and maximum values.}
\CommentTok{\# 4. Calculate the standard deviation.}
\CommentTok{\#}
\CommentTok{\# Write your code below:}
...
\CommentTok{\# ============================================================}
\CommentTok{\# }\AlertTok{TASK}\CommentTok{ 3: Build a small portfolio data set}
\CommentTok{\# ============================================================}
\CommentTok{\#}
\CommentTok{\# Your investment company is considering five companies:}
\CommentTok{\#}
\CommentTok{\# Company A}
\CommentTok{\# Company B}
\CommentTok{\# Company C}
\CommentTok{\# Company D}
\CommentTok{\# Company E}
\CommentTok{\#}
\CommentTok{\# The current share prices and annual returns are:}
\CommentTok{\#}
\CommentTok{\#             Price (NOK)     Return (\%)}
\CommentTok{\# Company A       20              5}
\CommentTok{\# Company B       35             {-}2}
\CommentTok{\# Company C       15              8}
\CommentTok{\# Company D       50              3}
\CommentTok{\# Company E       40              6}
\CommentTok{\#}
\CommentTok{\# 1. Create a data frame called \textasciigrave{}companies\textasciigrave{}.}
\CommentTok{\# 2. Display the data frame.}
\CommentTok{\# 3. Use \textasciigrave{}summary()\textasciigrave{} to get a summary of the data.}
\CommentTok{\# 4. Use \textasciigrave{}str()\textasciigrave{} to inspect the structure of the data frame.}
\CommentTok{\#}
\CommentTok{\# Write your code below:}
...
\CommentTok{\# ============================================================}
\CommentTok{\# }\AlertTok{TASK}\CommentTok{ 4: Identify the best and worst performers}
\CommentTok{\# ============================================================}
\CommentTok{\#}
\CommentTok{\# The manager wants to know:}
\CommentTok{\#}
\CommentTok{\# "Which company had the highest return?"}
\CommentTok{\# "Which company had the lowest return?"}
\CommentTok{\#}
\CommentTok{\# Using the \textasciigrave{}companies\textasciigrave{} data frame:}
\CommentTok{\#}
\CommentTok{\# 1. Find the highest return.}
\CommentTok{\# 2. Find the lowest return.}
\CommentTok{\# 3. Identify the company with the highest return.}
\CommentTok{\# 4. Identify the company with the lowest return.}
\CommentTok{\#}
\CommentTok{\# Write your code below:}
...
\CommentTok{\# ============================================================}
\CommentTok{\# }\AlertTok{TASK}\CommentTok{ 5: Visualise the companies}
\CommentTok{\# ============================================================}
\CommentTok{\#}
\CommentTok{\# A picture can make it easier to compare companies.}
\CommentTok{\#}
\CommentTok{\# Create a scatter plot with:}
\CommentTok{\#}
\CommentTok{\#     x{-}axis = Share price}
\CommentTok{\#     y{-}axis = Annual return}
\CommentTok{\#}
\CommentTok{\# 1. Create the scatter plot using \textasciigrave{}plot()\textasciigrave{}.}
\CommentTok{\# 2. Add a meaningful x{-}axis label.}
\CommentTok{\# 3. Add a meaningful y{-}axis label.}
\CommentTok{\# 4. Add a meaningful title.}
\CommentTok{\#}
\CommentTok{\# Write your code below:}
...
\CommentTok{\# After looking at the plot, answer:}
\CommentTok{\#}
\CommentTok{\# Do companies with higher share prices appear to have higher}
\CommentTok{\# returns in this small sample?}
\CommentTok{\#}
...
\CommentTok{\# ============================================================}
\CommentTok{\# }\AlertTok{TASK}\CommentTok{ 6: Install and load packages}
\CommentTok{\# ============================================================}
\CommentTok{\#}
\CommentTok{\# R itself comes with a small set of functions. Everything else lives in a}
\CommentTok{\# package: you install it once onto your computer, then load it again in}
\CommentTok{\# every session where you want to use it.}
\CommentTok{\#}
\CommentTok{\# Install the following packages for this course:}
\CommentTok{\#}
\CommentTok{\#     tidyverse      data wrangling and plotting}
\CommentTok{\#     data.table     fast handling of larger data sets}
\CommentTok{\#     moments        summary statistics}
\CommentTok{\#     bookdown       knitting reports to PDF}
\CommentTok{\#     tinytex        the LaTeX engine that PDF knitting needs}
\CommentTok{\#}
\CommentTok{\# 1. Install all five with a single call to \textasciigrave{}install.packages()\textasciigrave{}.}
\CommentTok{\# 2. \textasciigrave{}tinytex\textasciigrave{} is only the manager, not LaTeX itself. Install the LaTeX}
\CommentTok{\#    distribution by running \textasciigrave{}tinytex::install\_tinytex()\textasciigrave{}.}
\CommentTok{\# 3. Load \textasciigrave{}tidyverse\textasciigrave{} and \textasciigrave{}moments\textasciigrave{} with \textasciigrave{}library()\textasciigrave{}.}
\CommentTok{\# 4. Check that loading worked: run \textasciigrave{}skewness(c(1, 2, 3, 10))\textasciigrave{}. That}
\CommentTok{\#    function exists only once \textasciigrave{}moments\textasciigrave{} has been loaded.}
\CommentTok{\# 5. Check that LaTeX is in place: run \textasciigrave{}tinytex::is\_tinytex()\textasciigrave{}. It should}
\CommentTok{\#    return TRUE.}
\CommentTok{\#}
\CommentTok{\# You only need to install packages once. They will be available in all future R sessions. You only need to load them with \textasciigrave{}library()\textasciigrave{} in each new session.}
\CommentTok{\#}
\CommentTok{\# Run the code below. The two installation lines go in the Console; the rest}
\CommentTok{\# belong in the script, since loading has to happen in every session.}

\FunctionTok{install.packages}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{, }\StringTok{"moments"}\NormalTok{, }\StringTok{"data.table"}\NormalTok{, }\StringTok{"bookdown"}\NormalTok{, }\StringTok{"tinytex"}\NormalTok{))}
\NormalTok{tinytex}\SpecialCharTok{::}\FunctionTok{install\_tinytex}\NormalTok{()}

\FunctionTok{library}\NormalTok{(tidyverse)}
\FunctionTok{library}\NormalTok{(moments)}

\FunctionTok{skewness}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{10}\NormalTok{))}
\NormalTok{tinytex}\SpecialCharTok{::}\FunctionTok{is\_tinytex}\NormalTok{()}


\CommentTok{\# Copy and paste the output of the last two lines below.}
\CommentTok{\# Output:}
\end{Highlighting}
\end{Shaded}

\chapter{Lab 0: Introduction to R}\label{lab-0-introduction-to-r}
...
\tightlist
\item
  R makes use of the \texttt{\#} sign to add comments, so that you and
  others can understand what the R code is about.
\item
  R is case sensitive, so \texttt{my\_apples} and \texttt{My\_Apples}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calculate 6 + 12}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Subtract 5 from 10}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Multiply 3 by 5}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Divide 15 by 3}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Take the square of 3}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Take the square root of 16}
\end{Highlighting}
\end{Shaded}

\section{Variable Assignments}\label{variable-assignments}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Assign the value 4 to a variable called my\_apples}
\NormalTok{my\_apples }\OtherTok{\textless{}{-}} \DecValTok{4}

\CommentTok{\# Print out the value of my\_apples}
\NormalTok{my\_apples}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Assign the value 5 to a variable called my\_oranges}

\CommentTok{\# Add the values of my\_apples and my\_oranges and assign the result to a variable called my\_fruit}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Decimal values like \texttt{4.5} are called \textbf{numerics}.
\item
  Whole numbers like \texttt{4} are called \textbf{integers}. Integers
  are also numerics.
\item
  Boolean values (\texttt{TRUE} or \texttt{FALSE}) are called
  \textbf{logical}.
\item
  Text (or string) values are called \textbf{characters}.
\end{itemize}

Use \texttt{class()} function to check the data type of a variable.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Assign "Nord University" to a variable called my\_school}

\CommentTok{\# Check class of my\_school}

\CommentTok{\# Check class of my\_apples}
\end{Highlighting}
\end{Shaded}

\section{DataCamp: Introduction to R}\label{datacamp-introduction-to-r}

Go to DataCamp's
\href{https://www.datacamp.com/courses/free-introduction-to-r?utm_medium=organic_social&utm_campaign=sharewidget&utm_content=coursedetailpage&utm_source=copy}{Introduction
to R} course to learn more about R programming. The course is for free
if you join the DataCamp Classroom I have created for this course
(Otherwise, you will see many contents are locked). The course is 4
hours long and will help you to know the basics of R programming.

Use your \textbf{Nord University email} (@nord.no or @student.nord.no)
to sign up for a free account and join the DataCamp Classroom using
\href{https://www.datacamp.com/groups/shared_links/7104062fbde7415c350e33d8d22c9d1c739434092786cf9a3cb1266d78a56b68}{THIS
LINK}. Contact me if you have any issues joining the classroom.

\chapter{Import Data from Local
Files}\label{import-data-from-local-files}
...
In business analytics, when you have a dataset and you want to analyze
it in R. The first step is to import the data into R. You can use the
command \texttt{read.csv()}, but you need to specify the correct path to
your data file. This can be tricky if you are not familiar with file
paths and directories.

An easier alternative is to use RStudio's \textbf{Import Dataset} tool,
which provides a graphical interface to select your data file and
automatically generates the code to import it. This is especially useful
for those who are new to R or prefer a visual approach.

Up to now you have loaded data by writing \texttt{read\_csv()} yourself.
RStudio also ships with a point-and-click \textbf{Import Dataset} tool
that writes that line of code for you.

We will use a small example dataset to illustrate the process. The
dataset is a CSV file with four rows of heights and weights. You may
click \href{data/comma.csv}{HERE} to download this file to your compter.

\end{tcolorbox}

\texttt{comma.csv} data preview:
...
\section{Environment setup}\label{environment-setup-1}

The ``\textbf{From Text (readr)}'' option uses the \texttt{readr}
package, so \texttt{readr} has to be installed.

\textbf{If you already have the \texttt{tidyverse}, you are done.}
\texttt{readr} is one of the core tidyverse packages, so installing
\texttt{tidyverse} installed \texttt{readr} along with it --- there is
nothing extra to do. That is the case if you followed
\hyperref[environment-setup]{Environment Setup}.

Otherwise, run in the RStudio \textbf{Console}:
...
\section{Import a CSV step by step}\label{import-a-csv-step-by-step}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  In the \textbf{Environment} pane, click ``\textbf{Import Dataset}''.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_button.png}}
\item
  Choose ``\textbf{From Text (readr)}''.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_readr.png}}

  The two text options differ in which code they write for you.
  ``\textbf{From Text (readr)}'' uses \texttt{read\_csv()} from the
  \texttt{readr} package (part of the \texttt{tidyverse}), and
  ``\textbf{From Text (base)}'' uses base R's \texttt{read.csv()}. This
  course uses \texttt{readr} throughout, so pick that one.
\item
  The \textbf{Import Text Data} window opens. Click
  ``\textbf{Browse\ldots{}}'' to find your file.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_browse.png}}
\item
  Your operating system's file explorer opens. Select the file you want
  --- here, \texttt{comma.csv}.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_select_file.png}}
\item
  The window now fills in. Four areas are worth reading before you do
  anything else.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_overview.png}}

  \begin{itemize}
  \tightlist
  \item
    \textbf{File/URL} --- the path to the file you picked.
  \item
    \textbf{Data Preview} --- how R is \emph{currently} parsing the
    file, with the type R has guessed under each column name. Check
    this: if a column you expect to be numeric shows up as
    \texttt{character}, something in the file is not what you think it
    is.
  \item
    \textbf{Import Options} --- the name the object will get, whether
    the first row holds the column names, the delimiter, how missing
    values are written, and so on.
  \item
    \textbf{Code Preview} --- the code that produces exactly the import
    you have configured. Every change you make above rewrites this box.
  \end{itemize}
\item
  \textbf{Optional: You can change a column type} by clicking the small
  arrow in its column header and choosing from the dropdown. Here
  \texttt{ht\_in} and \texttt{wt\_lbs} are being changed from
  \texttt{double} to \texttt{integer}, since heights and weights in this
  file are whole numbers.

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_integer.png}}

  The same menu has \texttt{Factor}, \texttt{Date} and \texttt{Skip},
  which saves you a separate cleaning step later. Watch the \textbf{Code
  Preview} box as you do this: a \texttt{col\_types} argument appears.
\item
  \textbf{Important step: Do {NOT} press the ``Import'' button.} Click
  the clipboard icon above the \textbf{Code Preview} box to copy the
  code, then press ``\textbf{Cancel}''. This makes your import
  reproducible. You can re-run the same code again later.
  ...
  \vspace{-3mm}\textbf{Why copy the code instead of clicking Import?}\vspace{3mm}

  If you click ``\textbf{Import}'', RStudio will load the data for your
  current session only. If you want to import the data again, you will
  have to go through the import dialog again. This is {\textbf{NOT}
  reproducible}, and hence not a good practice.

  \end{minipage}%
  \end{tcolorbox}

  \raisebox{-0.2em}{\includegraphics[width=1.0\textwidth]{images/rstudio_import_copy_and_paste.png}}
\item
  \textbf{Final Step:} Go back to your R script or Quarto document and
  paste. Run the pasted code as though you had typed it yourself.
\end{enumerate}

\section{The code you end up with}\label{the-code-you-end-up-with}
...
\NormalTok{comma }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}
    \StringTok{"\textasciitilde{}/Dropbox/Datasets/Height and Weight/comma.csv"}\NormalTok{,}
    \AttributeTok{col\_types =} \FunctionTok{cols}\NormalTok{(}\AttributeTok{ht\_in =} \FunctionTok{col\_integer}\NormalTok{(),}
                     \AttributeTok{wt\_lbs =} \FunctionTok{col\_integer}\NormalTok{())}
\NormalTok{    )}
\FunctionTok{View}\NormalTok{(comma)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{Path} will be specific to your computer and your file
  location.
\item
  \textbf{Drop \texttt{View(comma)}.} It opens the data viewer, which is
  handy once but does nothing useful inside a script or a rendered
  document.
\end{itemize}
...
Note what \texttt{col\_types} bought us: \texttt{ht\_in} and
\texttt{wt\_lbs} come in as \texttt{\textless{}int\textgreater{}} rather
than \texttt{\textless{}dbl\textgreater{}}, exactly as chosen in the
dialog.

\section{Exercise}\label{exercise-1}

Use the ``\textbf{Import Dataset}'' tool to load
\texttt{CASchools\_test\_score.csv}, the dataset used later in the
course. Download it from \href{data/CASchools_test_score.csv}{HERE}.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Open the file through \textbf{Import Dataset} → \textbf{From Text
  (readr)}.
\item
  In the \textbf{Data Preview}, set \texttt{county} and \texttt{grades}
  to type \texttt{Factor}.
\item
  Copy the code, cancel the dialog, and paste it into a script.
\item
  Run \texttt{str()} on the result and check that \texttt{county} and
  \texttt{grades} are factors.
\end{enumerate}
...
\begin{itemize}
\item
  \href{https://www.r4epi.com/chapters/rstudio_import_tool/rstudio_import_tool}{@Brad
  Cannell, RStudio's Data Import Tool}
\item
  \href{https://docs.posit.co/ide/user/ide/guide/data/data-local.html}{RSutdio
  User Guide: Local Data}

  You may find ``\textbf{Importing data from Excel files}'' in this
  guide.
\end{itemize}
...
A logistics provider records the delivery outcome of every shipment as
\emph{early}, \emph{on time}, or \emph{late}. For a randomly selected
shipment, the probability of arriving early is 30\% and the probability
of arriving on time is 50\%. What is the probability that the shipment
...
\begin{refsolution}
Let \(A=\{\text{early}\}\) and \(B=\{\text{on time}\}.\) A single
shipment cannot be early \textbf{and} on time, so the events are
mutually exclusive and \(P(A \text{ and } B)=0.\) Then \[
\begin{split}
P(A \text{ or } B) = P(A)+P(B)-P(A \text{ and } B)=0.30+0.50=0.80.
\end{split}
\]
...
selected customer engages with at least one of the two channels?
...
\(P(A) + P(B) - P(A \text{ and } B) = 35\% + 25\% - 10\% = 50\%\)
...
\textbf{Moments} are numerical measures that describe the shape and
characteristics of a probability distribution.

\textbf{k-th raw moment}: \(E[x^k]\)

\textbf{k-th central moment}: \(E[(x-\mu)^k]\)

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
Moment & What it Measures & Statistic \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{1st moment} & Location / center & Mean \\
\textbf{2nd moment} & Dispersion & Variance \\
\textbf{3rd moment} & Asymmetry & Skewness \\
\textbf{4th moment} & Tail thickness / peakedness & Kurtosis \\
\end{longtable}
}
...
\item
  ...
  \[
  \text{Var}(X) = \color[HTML]{00CC66}{\mathbb{E}[X^2] - (\mathbb{E}[X])^2}.
  \]
...
A logistics company records the number of late deliveries per shift. Let
\(X\) be the number of late deliveries on a randomly chosen shift:

\[
P(X=0)=0.6, \qquad P(X=1)=0.2, \qquad P(X=2)=0.2.
\]

Compute \(\text{Var}(X)\) using the shortcut formula
\(\mathbb{E}[X^2] - (\mathbb{E}[X])^2\) and the variance definition.
...
\begin{refsolution}
\textbf{Using the shortcut formula:} \[
\begin{split}
\mathbb{E}[X] &= 0(0.6)+1(0.2)+2(0.2)=0+0.2+0.4=0.6 \\
\mathbb{E}[X^2] &= 0^2(0.6)+1^2(0.2)+2^2(0.2)=0+0.2+0.8=1.0 \\
\text{Var}(X) &= \mathbb{E}[X^2] - (\mathbb{E}[X])^2 = 1.0 - 0.6^2 = 1.0 - 0.36 = 0.64
\end{split}
\] \textbf{Using the definition of variance:} \[
\begin{split}
\text{Var}(X) &= E(X-E(X))^2 \\
&= \sum_i P(X=x_i)(x_i-E(X))^2 \\
&= 0.6(0-0.6)^2 + 0.2(1-0.6)^2 + 0.2(2-0.6)^2 \\
&= 0.6(0.36) + 0.2(0.16) + 0.2(1.96) \\
&= 0.216 + 0.032 + 0.392 \\
&= 0.64 
\end{split}
\] Both methods give \(\text{Var}(X)=0.64\).
...
\subsubsection{Linear Transformations of Random
Variables}\label{linear-transformations-of-random-variables}

For a random variable \(X\) and constants \(a, b \in \mathbb{R}\), the
linear transformation \(Y = a + bX\) has:

\begin{itemize}
\tightlist
\item
  \textbf{Expectation:} \(E[Y] = a + bE[X]\)
\item
  \textbf{Variance:}
  \(\text{Var}(Y) = \text{Var}(a + bX) = b^2 \text{Var}(X)\)
\end{itemize}

More generally, for \(n\) constants \(a_i\) and random variables
\(X_i, i=1,\ldots,n\),:
...
A portfolio allocates 40\% to Asset A and 60\% to Asset B, so its return
is \(P = 0.4A + 0.6B\). The two assets have \[
\begin{aligned}
\mathbb{E}[A] &= 0.08, & \text{Var}(A) &= 0.04, \\
\mathbb{E}[B] &= 0.12, & \text{Var}(B) &= 0.09, \\
\text{Cov}(A,B) &= 0.01.
\end{aligned}
\] Compute the portfolio's expected return \(\mathbb{E}[P]\) and its
variance \(\text{Var}(P)\).
...
\begin{refsolution}
Expected return (weights pass straight through): \[
\begin{aligned}
\mathbb{E}[P] &= w_a E[A] + w_b E[B] \\
&= 0.4(0.08)+0.6(0.12)\\
&= 0.032+0.072 = 0.104,
\end{aligned}
\] i.e.~\(10.4\%\).

Variance (weights are squared, and the covariance term enters twice): \[
\begin{aligned}
\text{Var}(P) &= w_a^2\text{Var}(A) + w_b^2\text{Var}(B) + 2 w_a w_b \text{Cov}(A,B) \\
&= 0.4^2 (0.04) + 0.6^2(0.09) + 2(0.4)(0.6)(0.01) \\
&= 0.0064 + 0.0324 + 0.0048 \\
&= 0.0436,
\end{aligned}
\] so \(\text{sd}(P)=\sqrt{0.0436}\approx 0.209\), about \(20.9\%\).
...
A financial analyst uses a company's operating income and investment
income to forecast its future net income. Based on historical data, the
following forecasting model has been estimated: \[
F = 2 + 0.7X_1 + 0.3X_2,
\] where \(F\) is the forecasted net income, \(X_1\) is operating
income, and \(X_2\) is investment income. The constant and the
coefficients are estimated from historical data. If \[
\begin{aligned}
\operatorname{Var}(X_1) &= 25, \\
\operatorname{Var}(X_2) &= 9, \\
\operatorname{Cov}(X_1,X_2) &= 6,
\end{aligned}
\] calculate the variance of the final forecast,
\(\operatorname{Var}(F)\).
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{For the \(t\)-distribution:}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-caution-color!10!white, colframe=quarto-callout-caution-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Independence vs.~Correlation}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-caution-color!10!white, colframe=quarto-callout-caution-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Mutually Exclusive vs Independence}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Interpretation tip}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
Alternatively, copy and paste the R code into RStudio Desktop and run it
there.
...
Run each cell \textbf{sequentially}. Read the comments carefully.
...
\item
  ...
  We cannot say this because the Corr(X, Y) ≈ 0.02, which is not exactly
  zero.
\item
  ...
  This means there exists a linear correlation, but it is a weak one
...
\includegraphics[width=5in,height=4.375in]{01_Lab-1_Descriptive-Statistics_files/figure-pdf/cell-6-output-1.png}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/correlation_varying_rho.png}

}

\caption{correlation\_varying\_rho}
...
We have a variable \(X \sim N(0, 25),\) that is, \(X\) is from a normal
distribution with mean 0 and variance 25.

Normal distribution notation: \(X\sim N(\mu, \sigma^2)\) means that
\(X\) is normally distributed with mean \(\mu\) and variance
\(\sigma^2.\)

Note: The second parameter is the variance, {\textbf{NOT}} the standard
deviation.
...
A data.frame: 9 × 3
\begin{tabular}{lll}
 sample\_size & var\_biased & var\_unbiased\\
 <dbl> & <dbl> & <dbl>\\
\hline
        5 &  4.262707 &  5.328384\\
...
\item
  \textbf{How does the difference between the biased and unbiased sample
  variance estimators change as sample size increases?}
\end{itemize}
...
The figure below shows the how the sample variance estimates change as
the sample size increases. The true (population) variance is 25, shown
as the horizontal dashed line. The blue line is the biased sample
variance and the orange line is the unbiased sample variance.
...
\includegraphics[width=5in,height=4.375in]{01_Lab-1_Descriptive-Statistics_files/figure-pdf/cell-12-output-1.png}

\section{Correlated Asset Returns}\label{correlated-asset-returns}

We will use 2,000 days of data on the returns of two (correlated) asset
to examine the benfits of diversification.

You will first calculate the average volatility of the two assets and
compare it with that of an equally weighted portfolio.
...
\includegraphics[width=5in,height=4.375in]{01_Lab-1_Descriptive-Statistics_files/figure-pdf/cell-15-output-1.png}
...
\subsubsection{\texorpdfstring{{Practical
Exercise}}{Practical Exercise}}\label{practical-exercise}
...
Calculate the average portfolio returns. \[
...
district's schools serve kindergarten through 8th grade. \\
...
\item
  ...
  {\$3,900} to over {\$7,700} - this substantial variation will be a key
...
\tightlist
\item
  \textbf{Range:} Expenditure varies from about {\$3,926} to {\$7,712}
  per student - a difference of nearly {\$4,000}
\item
  \textbf{Central Tendency:} The mean ({\$5,312}) is slightly higher
  than the median ({\$5,196}), \textbf{suggesting a slight right skew}
\item
  \textbf{Variability:} Standard deviation of {\$633} indicates moderate
...
\tightlist
\item
  \textbf{K--6 schools} (61 districts): Mean expenditure of {\$5,577,}
  slightly higher than K--8 schools
\item
  \textbf{K--8 schools} (359 districts): Mean expenditure of {\$5,267,}
  with a larger sample size
\item
  K--6 schools indicating higher variability, with a standard deviation
  of {\$662} compared to {\$618} for K-8 schools
\item
...
\begin{verbatim}
      n   min mean median  max   sd skewness kurtosis
1 10000 -1.88 0.02  -0.16 4.76 1.01     0.88      3.6
\end{verbatim}
...
\begin{verbatim}
      n  min  mean median  max sd skewness kurtosis
1 10000 -4.7 -0.01   0.16 1.88  1    -0.84     3.47
\end{verbatim}
...
\begin{verbatim}
     n    min  mean median   max   sd skewness kurtosis
1 1000 -12.15 -0.12  -0.08 10.34 1.65    -0.57    11.61
\end{verbatim}
...
\begin{verbatim}
     n  min mean median  max  sd skewness kurtosis
1 1000 -1.3 0.03   0.04 1.16 0.6    -0.07     1.86
\end{verbatim}
...
\chapter{Lab 2: Data Visualization --- Asset
Returns}\label{lab-2-data-visualization-asset-returns}

Open in Google Colab:
\href{https://colab.research.google.com/drive/15klt5pDuQFp413jkLaUeh7rIH_WOlCfK?usp=share_link}{Link}
...
In this lab you will work with daily returns of six \textbf{sector
indices} on Oslo Børs, covering 2020 to 2025.
...
\tightlist
\item
  Describe a distribution with summary statistics, a box plot and a
  histogram
\item
  Recognize \textbf{fat tails} in real return data, and why they matter
  for risk
\item
...
Run the cell below once. It installs any missing packages and loads
them.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# load packages}
\NormalTok{pkgs }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{, }\StringTok{"scales"}\NormalTok{, }\StringTok{"moments"}\NormalTok{, }\StringTok{"data.table"}\NormalTok{, }\StringTok{"ggrepel"}\NormalTok{, }\StringTok{"ggsci"}\NormalTok{, }\StringTok{"knitr"}\NormalTok{, }\StringTok{"kableExtra"}\NormalTok{, }\StringTok{"IRdisplay"}\NormalTok{)}
\NormalTok{missing }\OtherTok{\textless{}{-}} \FunctionTok{setdiff}\NormalTok{(pkgs, }\FunctionTok{rownames}\NormalTok{(}\FunctionTok{installed.packages}\NormalTok{()))}
\ControlFlowTok{if}\NormalTok{ (}\FunctionTok{length}\NormalTok{(missing) }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{) }\FunctionTok{install.packages}\NormalTok{(missing)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{f\_url }\OtherTok{\textless{}{-}} \StringTok{"https://raw.githubusercontent.com/my1396/FIN5005/refs/heads/main/data/NO\_6{-}sector\_index\_daily.csv"}
\NormalTok{sector\_index }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(f\_url, }\AttributeTok{show\_col\_types =} \ConstantTok{FALSE}\NormalTok{)}
\FunctionTok{print}\NormalTok{(sector\_index}\SpecialCharTok{$}\NormalTok{sector }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{unique}\NormalTok{())}
\CommentTok{\# data preview, first and last 5 rows}
\NormalTok{sector\_index }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{as.data.table}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{print}\NormalTok{(}\AttributeTok{topn =} \DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The file contains daily returns for \textbf{six industry sectors} from
2020 to 2025. It has \textbf{9,396 rows} and four columns. One row is
\textbf{one sector on one trading day}, and every sector has all 1,566
trading days --- the panel is balanced.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{sector} & text & Sector name. Six sectors:
\texttt{Consumer\ Non-Cyclicals}, \texttt{Energy}, \texttt{Finance},
\texttt{Industrials}, \texttt{Non-Energy\ Materials},
\texttt{Technology}. \\
\texttt{p\_date} & date & Price date, i.e.~the trading day the return
refers to. Runs from 2020-01-01 to 2025-12-31. \\
\texttt{ret} & numeric & The sector index return \textbf{on that day},
as a \textbf{decimal}. \texttt{0.0001} means the index rose 1 basis
point that day. \\
\texttt{cum\_ret} & numeric & The \textbf{cumulative} return of the
index from 2020-01-01 up to and including that day, as a decimal.
\texttt{0.01} means 1 NOK invested at the start would be worth 1.01
NOK. \\
\end{longtable}
}

\section{Part 1 --- Summary statistics}\label{part-1-summary-statistics}

We will use the helper function to calculate summary statistics for each
sector.
...
\NormalTok{quick\_summary }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x) \{}
    \FunctionTok{data.frame}\NormalTok{(}
        \AttributeTok{n =} \FunctionTok{length}\NormalTok{(x), }\AttributeTok{min =} \FunctionTok{min}\NormalTok{(x), }\AttributeTok{mean =} \FunctionTok{mean}\NormalTok{(x), }\AttributeTok{median =} \FunctionTok{median}\NormalTok{(x),}
        \AttributeTok{max =} \FunctionTok{max}\NormalTok{(x), }\AttributeTok{sd =} \FunctionTok{sd}\NormalTok{(x),}
        \AttributeTok{skewness =}\NormalTok{ moments}\SpecialCharTok{::}\FunctionTok{skewness}\NormalTok{(x), }\AttributeTok{kurtosis =}\NormalTok{ moments}\SpecialCharTok{::}\FunctionTok{kurtosis}\NormalTok{(x),}
        \AttributeTok{row.names =} \ConstantTok{NULL}
\NormalTok{    )}
\NormalTok{\}}
\end{Highlighting}
...
\subsection[ Task 1.1 --- summary statistics by
sector]{\texorpdfstring{\protect\includegraphics[width=\linewidth,height=1em,keepaspectratio]{filters/emoji/270d.png}
Task 1.1 --- summary statistics by
sector}{ Task 1.1 --- summary statistics by sector}}\label{task-1.1-summary-statistics-by-sector}

Produce the summary statistics \textbf{for each sector}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{stats\_by\_sector }\OtherTok{\textless{}{-}}\NormalTok{ sector\_index }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(sector) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_modify}\NormalTok{(}\SpecialCharTok{\textasciitilde{}} \FunctionTok{quick\_summary}\NormalTok{(.x}\SpecialCharTok{$}\NormalTok{ret)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ungroup}\NormalTok{()}
\NormalTok{stats\_by\_sector}
\end{Highlighting}
\end{Shaded}
...
Q1: Return volatility/risk is often measured as the \textbf{standard
deviation} of daily returns. Which sector is the \textbf{least risky}
and which is the \textbf{most risky}?

A: {[}Type your answer here{]}

\textbf{Q2:} By looking at skewness, which sector has the most
\textbf{asymmetric} distribution of daily returns? By looking at
kurtosis, which sector has the \textbf{fattest tails}?

A: {[}Type your answer here{]}

Q3: Why the least risky sector is not necessarily the one with the
thinnest tails?

A: {[}Type your answer here{]}

\section{Part 2 --- Show the
distribution}\label{part-2-show-the-distribution}

Summary statistics compress a distribution into a handful of numbers,
and very different distributions can share the same mean and standard
deviation. Always look at the shape as well.

\subsection[ Task 2.1 --- box plot by
sector]{\texorpdfstring{\protect\includegraphics[width=\linewidth,height=1em,keepaspectratio]{filters/emoji/270d.png}
Task 2.1 --- box plot by
sector}{ Task 2.1 --- box plot by sector}}\label{task-2.1-box-plot-by-sector}

A box plot shows the median (the line inside the box), the interquartile
range (the box itself, Q1 to Q3), whiskers reaching to the bulk of the
data, and individual points for anything beyond. It is the fastest way
to compare several groups side by side.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# fix factor order}
\NormalTok{sector\_order }\OtherTok{\textless{}{-}}\NormalTok{ stats\_by\_sector }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{arrange}\NormalTok{(median) }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{pull}\NormalTok{(sector)}
\NormalTok{sector\_index }\OtherTok{\textless{}{-}}\NormalTok{ sector\_index }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{sector =} \FunctionTok{factor}\NormalTok{(sector, }\AttributeTok{levels =}\NormalTok{ sector\_order))}

\CommentTok{\# rename sector labels}
\NormalTok{labels }\OtherTok{\textless{}{-}} \FunctionTok{setNames}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"Consumer Staple"}\NormalTok{, }\StringTok{"Energy"}\NormalTok{, }\StringTok{"Industry"}\NormalTok{, }\StringTok{"Tech."}\NormalTok{, }\StringTok{"Non{-}Engergy"}\NormalTok{, }\StringTok{"Finance"}\NormalTok{), sector\_order)}

\FunctionTok{ggplot}\NormalTok{(sector\_index, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ sector, }\AttributeTok{y =}\NormalTok{ ret)) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.size =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{outlier.alpha =} \FloatTok{0.3}\NormalTok{, }\AttributeTok{fill =} \StringTok{"grey90"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{scale\_x\_discrete}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ labels) }\SpecialCharTok{+}
    \FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\NormalTok{percent) }\SpecialCharTok{+}
    \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Daily returns by sector"}\NormalTok{, }\AttributeTok{x =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{y =} \StringTok{"Daily return"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{theme\_minimal}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{14}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{theme}\NormalTok{(}
        \AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{angle =} \DecValTok{90}\NormalTok{, }\AttributeTok{vjust =} \DecValTok{1}\NormalTok{, }\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{)}
\NormalTok{    )}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=4.375in,height=4.375in]{02_Lab-2_Data_Visualization_Asset_Returns_files/figure-pdf/cell-7-output-1.png}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calculate IQR}
\NormalTok{sector\_IQR }\OtherTok{\textless{}{-}}\NormalTok{ sector\_index }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(sector) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{summarise}\NormalTok{(}\AttributeTok{IQR =} \FunctionTok{IQR}\NormalTok{(ret)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{arrange}\NormalTok{(}\FunctionTok{desc}\NormalTok{(IQR)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{left\_join}\NormalTok{(stats\_by\_sector[}\FunctionTok{c}\NormalTok{(}\StringTok{"sector"}\NormalTok{, }\StringTok{"sd"}\NormalTok{)], }\AttributeTok{by =} \StringTok{"sector"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\FunctionTok{across}\NormalTok{(IQR}\SpecialCharTok{:}\NormalTok{sd, }\SpecialCharTok{\textasciitilde{}} \FunctionTok{percent}\NormalTok{(.x, }\AttributeTok{accuracy =} \FloatTok{0.01}\NormalTok{)))}
\NormalTok{results }\OtherTok{\textless{}{-}}\NormalTok{ sector\_IQR }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{kable}\NormalTok{(}\AttributeTok{format =} \StringTok{"html"}\NormalTok{)}
\FunctionTok{display\_html}\NormalTok{(}\FunctionTok{as.character}\NormalTok{(results))}
\end{Highlighting}
\end{Shaded}
...
\textbf{Q4:} Which sector has the widest box, and which the narrowest?
Compare your reading of the plot with the \texttt{sd} column from Task
1.1 --- do they agree?

A: {[}Type your answer here{]}

\subsection[ Task 2.2 ---
histogram]{\texorpdfstring{\protect\includegraphics[width=\linewidth,height=1em,keepaspectratio]{filters/emoji/270d.png}
Task 2.2 ---
histogram}{ Task 2.2 --- histogram}}\label{task-2.2-histogram}

The box plot compares sectors; a histogram shows the shape of one
distribution in detail. Start with 50 bins.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sector\_data }\OtherTok{\textless{}{-}}\NormalTok{ sector\_index }\SpecialCharTok{\%\textgreater{}\%}
     \FunctionTok{filter}\NormalTok{(sector }\SpecialCharTok{==} \StringTok{"Consumer Non{-}Cyclicals"}\NormalTok{)}

\FunctionTok{ggplot}\NormalTok{(sector\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ ret)) }\SpecialCharTok{+}
     \FunctionTok{geom\_histogram}\NormalTok{(}
          ...
          \AttributeTok{fill =} \StringTok{"\#2a78d6"}\NormalTok{,}
...
\NormalTok{     ) }\SpecialCharTok{+}
     \FunctionTok{stat\_function}\NormalTok{(}
          ...
          \AttributeTok{args =} \FunctionTok{list}\NormalTok{(}
               \AttributeTok{mean =} \FunctionTok{mean}\NormalTok{(sector\_data}\SpecialCharTok{$}\NormalTok{ret, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
               \AttributeTok{sd =} \FunctionTok{sd}\NormalTok{(sector\_data}\SpecialCharTok{$}\NormalTok{ret, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{          ),}
          \AttributeTok{colour =} \StringTok{"\#eb6834"}\NormalTok{,}
          \AttributeTok{linewidth =} \FloatTok{0.8}
\NormalTok{     ) }\SpecialCharTok{+}
     \FunctionTok{scale\_x\_continuous}\NormalTok{(}\AttributeTok{limits=}\FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{0.1}\NormalTok{,}\FloatTok{0.1}\NormalTok{),}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\NormalTok{percent) }\SpecialCharTok{+}
     \FunctionTok{labs}\NormalTok{(}
          \AttributeTok{title =} \StringTok{"Distribution of daily sector index returns"}\NormalTok{,}
          \AttributeTok{subtitle =} \StringTok{"Orange curve = normal density benchmark; dashed line = mean"}\NormalTok{,}
...
\includegraphics[width=4.375in,height=4.375in]{02_Lab-2_Data_Visualization_Asset_Returns_files/figure-pdf/cell-9-output-2.png}

\subsection[ Task 2.3 --- QQ
plot]{\texorpdfstring{\protect\includegraphics[width=\linewidth,height=1em,keepaspectratio]{filters/emoji/270d.png}
Task 2.3 --- QQ plot}{ Task 2.3 --- QQ plot}}\label{task-2.3-qq-plot}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# plot the QQ plot for Consumer Non{-}Cyclicals}
\FunctionTok{ggplot}\NormalTok{(sector\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ ret)) }\SpecialCharTok{+}
     \FunctionTok{stat\_qq}\NormalTok{(}\AttributeTok{size =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.5}\NormalTok{) }\SpecialCharTok{+}
     \FunctionTok{stat\_qq\_line}\NormalTok{(}\AttributeTok{colour =} \StringTok{"\#eb6834"}\NormalTok{, }\AttributeTok{linewidth =} \FloatTok{0.8}\NormalTok{) }\SpecialCharTok{+}
     ...
     \FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\NormalTok{percent) }\SpecialCharTok{+} 
     \FunctionTok{labs}\NormalTok{(}
          \AttributeTok{title =} \StringTok{"QQ plot of daily sector index returns"}\NormalTok{,}
          \AttributeTok{subtitle =} \StringTok{"Orange line = normal distribution benchmark"}\NormalTok{,}
...
\includegraphics[width=4.375in,height=4.375in]{02_Lab-2_Data_Visualization_Asset_Returns_files/figure-pdf/cell-10-output-1.png}

\textbf{Q5:} What can you tell from the QQ plot about the tails of the
distribution?

A: {[}Type your answer here{]}

\section{Part 3 --- Cumulative return}\label{part-3-cumulative-return}

\subsection{How to calculate it}\label{how-to-calculate-it}

Every return so far has been a \textbf{one-day} return. To answer ``what
would 1 NOK invested on 2020-01-01 be worth today?'' you must
\textbf{compound} the daily returns --- you cannot simply add them up.

A return of \(r_t\) turns 1 NOK into \((1 + r_t)\) NOK. Over \(T\) days
you multiply those growth factors together:

\[
1 \;\longrightarrow\; (1+r_1)(1+r_2)\cdots(1+r_T) \;=\; \prod_{t=1}^{T} (1 + r_t)
\]

That product is the \textbf{gross} cumulative return --- the value of
your 1 NOK. Subtract the 1 NOK you started with to get the \textbf{net}
cumulative return:
...
In the dataset, the \texttt{cum\_ret} column is the \textbf{net}
cumulative return, expressed as a decimal.

\subsection[ Task 4.1 Plot the cumulative
return]{\texorpdfstring{\protect\includegraphics[width=\linewidth,height=1em,keepaspectratio]{filters/emoji/270d.png}
Task 4.1 Plot the cumulative
return}{ Task 4.1 Plot the cumulative return}}\label{task-4.1-plot-the-cumulative-return}

Draw one line per sector.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sector\_cols }\OtherTok{\textless{}{-}} \FunctionTok{setNames}\NormalTok{(}\FunctionTok{pal\_d3}\NormalTok{()(}\DecValTok{6}\NormalTok{), sector\_order)}
\CommentTok{\# the last observation of each sector, used to place the labels}
\NormalTok{ends }\OtherTok{\textless{}{-}}\NormalTok{ sector\_index }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{group\_by}\NormalTok{(sector) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{slice\_tail}\NormalTok{(}\AttributeTok{n =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{ungroup}\NormalTok{()}

\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(sector\_index, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ p\_date, }\AttributeTok{y =}\NormalTok{ cum\_ret, }\AttributeTok{colour =}\NormalTok{ sector)) }\SpecialCharTok{+}
     \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =} \DecValTok{0}\NormalTok{, }\AttributeTok{colour =} \StringTok{"grey70"}\NormalTok{, }\AttributeTok{linewidth =} \FloatTok{0.3}\NormalTok{) }\SpecialCharTok{+}
     ...
     \FunctionTok{geom\_text\_repel}\NormalTok{(}
          \AttributeTok{data =}\NormalTok{ ends, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{label =}\NormalTok{ sector), }\AttributeTok{hjust =} \DecValTok{0}\NormalTok{, }\AttributeTok{direction =} \StringTok{"y"}\NormalTok{,}
          \AttributeTok{nudge\_x =} \DecValTok{120}\NormalTok{, }\AttributeTok{segment.colour =} \StringTok{"grey70"}\NormalTok{, }\AttributeTok{size =} \DecValTok{4}\NormalTok{, }\AttributeTok{seed =} \DecValTok{1}
\NormalTok{     ) }\SpecialCharTok{+}
     \FunctionTok{scale\_colour\_manual}\NormalTok{(}\AttributeTok{values =}\NormalTok{ sector\_cols) }\SpecialCharTok{+}
     \FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =}\NormalTok{ scales}\SpecialCharTok{::}\NormalTok{percent) }\SpecialCharTok{+}
     \FunctionTok{scale\_x\_date}\NormalTok{(}
          \AttributeTok{breaks =} \FunctionTok{seq}\NormalTok{(}\FunctionTok{as.Date}\NormalTok{(}\StringTok{"2020{-}01{-}01"}\NormalTok{), }\FunctionTok{as.Date}\NormalTok{(}\StringTok{"2025{-}01{-}01"}\NormalTok{), }\AttributeTok{by =} \StringTok{"1 year"}\NormalTok{),}
          \AttributeTok{date\_labels =} \StringTok{"\%Y"}\NormalTok{, }\AttributeTok{expand =} \FunctionTok{expansion}\NormalTok{(}\AttributeTok{mult =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.02}\NormalTok{, }\FloatTok{0.26}\NormalTok{))}
\NormalTok{     ) }\SpecialCharTok{+}
     \FunctionTok{labs}\NormalTok{(}
          \AttributeTok{title =} \StringTok{"Cumulative return of value{-}weighted sector indices"}\NormalTok{,}
          \AttributeTok{subtitle =} \StringTok{"Oslo Børs, 2020{-}01{-}01 to 2025{-}12{-}31"}\NormalTok{,}
          \AttributeTok{x =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{y =} \StringTok{"Cumulative return"}
\NormalTok{     ) }\SpecialCharTok{+}
     \FunctionTok{theme\_minimal}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{14}\NormalTok{) }\SpecialCharTok{+}
     \FunctionTok{theme}\NormalTok{(}\AttributeTok{panel.grid.minor =} \FunctionTok{element\_blank}\NormalTok{(), }\AttributeTok{legend.position =} \StringTok{"none"}\NormalTok{)}

\CommentTok{\# Save the plot to a file}
\NormalTok{f\_name }\OtherTok{\textless{}{-}} \StringTok{"images/index\_cum\_returns\_line{-}chart.png"}
\ControlFlowTok{if}\NormalTok{ (}\SpecialCharTok{!}\FunctionTok{dir.exists}\NormalTok{(}\StringTok{"images"}\NormalTok{)) }\FunctionTok{dir.create}\NormalTok{(}\StringTok{"images"}\NormalTok{)}
\FunctionTok{ggsave}\NormalTok{(f\_name, p, }\AttributeTok{width =} \DecValTok{12}\NormalTok{, }\AttributeTok{height =} \DecValTok{6}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/index_cum_returns_line-chart.png}

\textbf{Q6:} Every sector has one obvious collapse early in the sample.
Which sector fell furthest, and what event does the date correspond to?
Use the code below to check your reading of the chart against the
numbers.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sector\_index }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(sector) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{summarise}\NormalTok{(}\AttributeTok{trough =} \FunctionTok{min}\NormalTok{(cum\_ret), }\AttributeTok{trough\_date =}\NormalTok{ p\_date[}\FunctionTok{which.min}\NormalTok{(cum\_ret)]) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\FunctionTok{across}\NormalTok{(trough, }\SpecialCharTok{\textasciitilde{}} \FunctionTok{round}\NormalTok{(.x, }\DecValTok{4}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}

A tibble: 6 × 3
\begin{tabular}{lll}
 sector & trough & trough\_date\\
 <fct> & <dbl> & <date>\\
\hline
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Key Components of the Linear Regression Model}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\item
  ...
  increase by \textbf{0.0057 points}. Put differently: a
  \textbf{{\$1,000} increase} in expenditure is associated with a
  \textbf{5.75-point increase} in test scores.
\end{itemize}
...
average, an additional {\$1,000} in per-student expenditure is
associated with an increase of about \textbf{5.75 points} in test
...
different possible random samples, \((X_i, Y_i).\) In small samples,
theses sampling distributions are complicated, but in {\textbf{large
samples}}, they are normal because of the \textbf{Central Limit Theorem}
(CLT).
...
\protect\phantomsection\label{clt1}
...
\protect\phantomsection\label{clt2}
...
\protect\phantomsection\label{clt3}
...
\protect\phantomsection\label{clt4}
\begin{itemize}
\item
...
\] where \[
\begin{aligned}
\hat{\bbeta} &= \left(\sum_{i=1}^n X_iX_i'\right)^{-1} \left(\sum_{i=1}^n X_iY_i\right), \text{ and} \\
...
\begin{itemize}
\item
  Again, note the scaling by \(\sqrt{n}.\) As sample size \(n\)
  increases, the \(\var(\hat{\bbeta})\) decreases, meaning that there is
  less uncertainty about the estimate of \(\bbeta\).
\item
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-important-color!10!white, colframe=quarto-callout-important-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title={Study objectives for hypothesis testing}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\item
  ...
  \begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Finding critical value}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\protect\phantomsection\label{solution2}
...
\protect\phantomsection\label{solution3}
...
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{images/F-dist_static.png}
\end{center}
...
\begin{aligned}
  (I) \; TestScore=& \, \beta_0 + \beta_1 \times STR + u, \\
  (II) \; TestScore=& \, \beta_0 + \beta_1 \times STR + \beta_2 \times english + u, \\
  (III) \; TestScore=& \, \beta_0 + \beta_1 \times STR + \beta_2 \times english + \beta_3 \times lunch + u, \\
  (IV) \; TestScore=& \, \beta_0 + \beta_1 \times STR + \beta_2 \times english + \beta_4 \times calworks + u, \\
  (V) \; TestScore=& \, \beta_0 + \beta_1 \times STR + \beta_2 \times english + \beta_3 \times lunch + \beta_4 \times calworks + u.
\end{aligned}
\]
...
\chapter{Lab 3: Regression with Dummy
Variables}\label{lab-3-regression-with-dummy-variables}
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{images/change-working-directory-r.png}

}

\caption{change-working-directory-r}
...
\begin{equation}\protect\phantomsection\label{eq-binary-regression}{
...
\begin{equation}\protect\phantomsection\label{eq-dummy-variable}{
D_i = 
\begin{cases}
...
\includegraphics[width=4.375in,height=4.375in]{07_Lab-3_dummy-variable_files/figure-pdf/cell-14-output-1.png}
...
\begin{equation}\protect\phantomsection\label{eq-binary-interaction}{
...
\begin{equation}\protect\phantomsection\label{eq-binary-interaction-more-controls}{
...
\item
  ...
  \(X\). For example, increasing income from {\$20,000} to {\$30,000}
  likely has a different effect on mortgage approval than increasing
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Important Distinction}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-warning-color!10!white, colframe=quarto-callout-warning-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title={Shortcomings with the linear probability model}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
