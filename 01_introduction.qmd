...
::: {.example #exm-1}
A logistics provider records the delivery outcome of every shipment as *early*, *on time*, or *late*. For a randomly selected shipment, the probability of arriving early is 30% and the probability of arriving on time is 50%. What is the probability that the shipment arrives early or on time?
:::

<button class="solution-btn" onclick="myFunction('sol-1')">Solution</button>
<div id="sol-1" class="solution-answer env-green">
Let $A=\{\text{early}\}$ and $B=\{\text{on time}\}.$ A single shipment cannot be early **and** on time, so the events are mutually exclusive and $P(A \text{ and } B)=0.$ Then
$$
\begin{split}
...
::: {.example #exm-2}
In a multi-channel marketing campaign, 35% of customers click an email offer (Event A) and 25% engage with a social media ad (Event B). Data shows 10% of customers do both. What is the probability a randomly selected customer engages with at least one of the two channels?
:::

<button class="solution-btn" onclick="myFunction('sol-2')">Solution</button>
<div id="sol-2" class="solution-answer env-green">$P(A) + P(B) - P(A \text{ and } B) = 35\% + 25\% - 10\% = 50\%$</div>
...
| -------------- | --------------------------- | -------------- |
| **1st moment** | Location / center           | Mean           |
| **2nd moment** | Dispersion                  | Variance       |
| **3rd moment** | Asymmetry                   | Skewness       |
| **4th moment** | Tail thickness / peakedness | Kurtosis       |
...
- **Variance (population):** How much values fluctuate around the true mean.
  ...
  $$
  \text{Var}(X) = \color{#00CC66}{\mathbb{E}[X^2] - (\mathbb{E}[X])^2}.
  $$
...
::: {.example #exm-10}
A logistics company records the number of late deliveries per shift. Let $X$ be the number of late deliveries on a randomly chosen shift:

$$
P(X=0)=0.6, \qquad P(X=1)=0.2, \qquad P(X=2)=0.2.
$$

Compute $\text{Var}(X)$ using the shortcut formula $\mathbb{E}[X^2] - (\mathbb{E}[X])^2$ and the variance definition. 
:::
...
::: {.example #exm-4}
A business has daily profits of [\$200, \$250, \$180, \$220, and \$210]{.mathjax_ignore}. Calculate the mean and variance.
:::
...
For a random variable $X$ and constants $a, b \in \mathbb{R}$, the linear transformation $Y = a + bX$ has:

- **Expectation:** $E[Y] = a + bE[X]$
- **Variance:** $\text{Var}(Y) = \text{Var}(a + bX) = b^2 \text{Var}(X)$

More generally, for $n$ constants $a_i$ and random variables $X_i, i=1,\ldots,n$,:
...
::: {.example #exm-12}
A portfolio allocates 40% to Asset A and 60% to Asset B, so its return is $P = 0.4A + 0.6B$. The two assets have
$$
\begin{aligned}
...
\end{aligned}
$$
Compute the portfolio's expected return $\mathbb{E}[P]$ and its variance $\text{Var}(P)$.
:::

<button class="solution-btn" onclick="myFunction('sol-12')">Solution</button>
<div id="sol-12" class="solution-answer env-green">
...
::: {.example #exm-13}
A financial analyst uses a company's operating income and investment income to forecast its future net income. Based on historical data, the following forecasting model has been estimated:
$$
F = 2 + 0.7X_1 + 0.3X_2,
$$
where \(F\) is the forecasted net income, \(X_1\) is operating income, and \(X_2\) is investment income. The constant and the coefficients are estimated from historical data.
If
$$
\begin{aligned}
...
\end{aligned}
$$
calculate the variance of the final forecast, \(\operatorname{Var}(F)\).
:::
