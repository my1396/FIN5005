...
P\left(-c_{1-\alpha/2} < \frac{\widehat{\theta}-\theta}{\Sigma} < c_{1-\alpha/2}  \right) = 1-\alpha .
\end{equation}
$$
This requires $\Phi(c_{1-\alpha/2}) = 1-\frac{\alpha}{2}$. 

Hence, $c_{1-\alpha/2}=\Phi^{-1}(1-\frac{\alpha}{2})$, which is the $\left(1-\frac{\alpha}{2}\right)$ quantile of the standard normal distribution (inverse of cdf of standard normal).  

Re-arranging \@ref(eq:CI), we have 
$$
P\left(\widehat{\theta}-c_{1-\alpha/2}\cdot \Sigma, \; \widehat{\theta}+c_{1+\alpha/2}\cdot \Sigma\right) = 1-\alpha
$$
This says the random interval, 
$$
(\widehat{\theta}-c_{1-\alpha/2}\cdot \Sigma, \; \widehat{\theta}+c_{1-\alpha/2}\cdot \Sigma),
$$
...
$$
P\left(\widehat{\theta}-c_{1-\alpha}\cdot \Sigma < \theta \right) = 1-\alpha ,
$$
or
$$
P\left(\theta < \widehat{\theta}+c_{1-\alpha}\cdot \Sigma \right) = 1-\alpha ,
$$
...
- we reject $H_0: \theta=r$ in favor of $H_1$ at $\alpha$ level of significance if 
    $$
    |Z| = \left\vert \frac{\widehat{\theta}-r}{\Sigma} \right\vert > c_{1-\alpha/2},
    $$
    where the corresponding cut-off value $c_{\alpha/2}$ is called the <span style='color:#008B45FF'>critical value</span>. $c_{1-\alpha/2}$ is defined as 
    $$
    \begin{aligned}
    & P(Z>c_{1-\alpha/2}) = \alpha/2 , \\
    \Rightarrow \, & P(Z\le c_{1-\alpha/2}) = 1-\alpha/2.
    \end{aligned}
    $$
    That is, $c_{1-\alpha/2}$ is the $1-\alpha/2$ quantile (inverse cdf) of the distribution of $Z$.
...
- Case 1: $H_0: \theta\ge r \quad \text{vs} \quad H_1: \theta<r$ (left-tailed test). We reject $H_0$ in favor of $H_1$ if 
$$
\frac{\widehat{\theta}-r}{\Sigma} < -c_{1-\alpha} \; ,
$$
that is, the critical region is in the extreme left region (tail).
Otherwise, we fail to reject $H_0.$

- Case 2: $H_0: \theta\le r \quad \text{vs} \quad H_1: \theta>r$ (right-tailed test). We reject $H_0$ in favor of $H_1$ if 
$$
\frac{\widehat{\theta}-r}{\Sigma} > c_{1-\alpha} \; ,
$$
...
- For one-sided tests,
    - 1\% level of significance : $c_{1-\alpha} = 2.33$,
    - 5\% level of significance : $c_{1-\alpha} = 1.64$, 
    - 10\% level of significance : $c_{1-\alpha}= 1.28$.

- For two-sided tests,
    - 1\% level of significance : $c_{1-\alpha/2} = 2.58$,
        ```{r}
        alpha <- .01
        qnorm(p=1-alpha/2)
        ```
    
    - 5\% level of significance : $\color{#337ab7}{c_{1-\alpha/2} = 1.96}$, 
        ```{r}
        alpha <- .05
        qnorm(p=1-alpha/2)
        ```
    
    - 10\% level of significance : $c_{1-\alpha/2}= 1.64$.
        ```{r}
        alpha <- .1
        qnorm(p=1-alpha/2)
        ```
