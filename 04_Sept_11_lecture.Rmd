...
- Given a random sample of $n=81$ customers, what is the probability of the average time spent by the 64 customers exceeds 42 minutes, i.e., compute $P(\overline{X}_{64} > 42)$?
...
$$
\text{Standard Error} = \frac{15}{\sqrt{81}} = \frac{15}{9} = 1.6667
$$

$$
Z = \frac{42 - 40}{1.6667} =  1.2
$$

$$
P(\overline{X} > 42) = P(Z > 1.2) \approx 0.1151
$$
A random sample of 81 customers has a **11.51%** chance of yielding an average time above 42 minutes.
...
sample_64 <- dnorm(x, mean = mu, sd = sigma / sqrt(n1))
sample_81 <- dnorm(x, mean = mu, sd = sigma / sqrt(n2))

# Create data frame
df <- data.frame(
  x = rep(x, 3),
  density = c(individual, sample_64, sample_81),
  group = factor(rep(c("Individual (n=1)", "Sample Mean (n=64)", "Sample Mean (n=81)"), each = length(x)))
)
