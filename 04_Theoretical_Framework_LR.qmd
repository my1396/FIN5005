...
The sampling distribution describes the values they could take over different possible random samples, $(X_i, Y_i).$ In small samples, theses sampling distributions are complicated, but in <span class="env-green">**large samples**</span>, they are normal because of the **Central Limit Theorem** (CLT).
...
- Again, note the scaling by $\sqrt{n}.$ As sample size $n$ increases, the $\var(\hat{\bbeta})$ decreases, meaning that there is less uncertainty about the estimate of $\bbeta$.
